...
# Reuse one event loop for all tests in the test session
# so that we won't have the event loop closed issue when
# closing websocket connection in another test.
...
async def session(capabilities, configuration, request, bidi):
    ...
    is_cur_bidi = type(_current_session) == webdriver.BidiSession
